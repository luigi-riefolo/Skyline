...
typedef struct _RTREEMBR {
    ...
    bool operator==(const _RTREEMBR lhs) {
      //minDist = bound[0] + bound[1] + bound[2];
      return lhs.bound == bound ;
	}
	bool operator>(const _RTREEMBR lhs) {
    return bound < lhs.bound ;
	}
	bool operator<(const _RTREEMBR lhs) {
    return bound > lhs.bound ;
	}
...
typedef struct _RTREENODE {
    ...
    bool operator==(const _RTREENODE lhs) {             /// NEED TO IMPLEMENT THIS!!!
      return lhs.branch == branch ;
	}
	bool operator>(const _RTREENODE lhs) {
    return lhs.branch > branch ;
	}
	bool operator<(const _RTREENODE lhs) {
    return lhs.branch < branch;
...
/**
 * Print out the data for a rectangle with name.
 */
string RTreeGetRectName( RTREEMBR *rc, int depth);
/**
...
/**
 * Decide whether rectangle rc1 dominates rc2.
 */
int RTreeDominate( RTREEMBR *rc1, RTREEMBR *rc2);
...
/**
 * Decide whether rectangle r is contained in rectangle s (modified version).
 */
int RTreeContain( RTREEMBR *r, RTREEMBR *s);
/**
 * Split a node.
 ...
 * Print out the data for a rectangle.
 */
void RTreePrintRect( RTREEMBR *rc, int depth)         // depth=spaces on line
{
    int i;
    ...
    {
        _RTreeTabIn(depth+1);
        fprintf (stdout, "%d", (int)rc->bound[i]);
    }
    printf("\n");
}
...
string RTreeGetRectName( RTREEMBR *rc, int depth) {
    ...
    _RTreeTabIn(depth);
    int x = (int)rc->bound[0];
    str<<"Point " <<rc->data<<"("<<(int)rc->bound[0]<<","<<(int)rc->bound[1]<<","<<(int)rc->bound[2]<<")";
...
int RTreeOverlap( RTREEMBR *rc1, RTREEMBR *rc2)
{
    ...
    for (i=0; i<DIMS_NUMB; i++)
    {
        ...
        if (rc1->bound[i] > rc2->bound[j] || rc2->bound[i] > rc1->bound[j] || 
        	(rc1->bound[0] == rc2->bound[0] && rc1->bound[1]==rc2->bound[1] &&
        	 rc1->bound[2] == rc2->bound[2]))
            return FALSE;
    }
    return TRUE;
}
...
int RTreeDominate( RTREEMBR *rc1, RTREEMBR *rc2) {
    ...
    assert(rc1 && rc2);
    // If rc1.x == rc2.x & rc1.y < rc2.y does it dominate????
    /*if ((rc1->bound[0]<rc2->bound[0] & rc1->bound[1]<rc2->bound[1] & rc1->bound[2]<rc2->bound[2])||     
 
       (rc1->bound[0]<rc2->bound[0] & rc1->bound[1]<rc2->bound[1]) ||
       (rc1->bound[0]<rc2->bound[0] & rc1->bound[2]<rc2->bound[2]) ||
       (rc1->bound[1]<rc2->bound[1] & rc1->bound[2]<rc2->bound[2]) ||
      /* (rc1->bound[0]<rc2->bound[0] & rc1->bound[1]==rc2->bound[1] & rc1->bound[2]==rc2->bound[2]) ||
       (rc1->bound[0]==rc2->bound[0] & rc1->bound[1]<rc2->bound[1] & rc1->bound[2]==rc2->bound[2]) ||
       (rc1->bound[0]==rc2->bound[0] & rc1->bound[1]==rc2->bound[1] & rc1->bound[2]<rc2->bound[2]) ||
       (rc1->bound[0]>rc2->bound[0] & rc1->bound[1]<rc2->bound[1] & rc1->bound[2]>rc2->bound[2]) ) {
       */
...
int RTreeContain( RTREEMBR *r, RTREEMBR *s) {
    ...
    // If same rectangle return false
    if (r->bound[0]==s->bound[0] & r->bound[1]==s->bound[1] & r->bound[1]==s->bound[1])
      return FALSE;
    /* undefined mbr is contained in any other */
    if (INVALID_RECT(r))
        return TRUE;
...
#pragma warning(pop)
    ...
    for (i=0; i<node->count; i++)
    {
        if(node->level == 0) {
             _RTreeTabIn(depth);
             int temp = node->branch[i].child;
             fprintf (stdout, " %d: data = %d ", i, temp-10); 
             RTreePrintRect(&node->branch[i].mbr,0);
        }
        else {
            _RTreeTabIn(depth);
            fprintf (stdout, "\nbranch %d \n", i);
